...
//code
	...
	/* Don't write main().
        ...
        int* incoutp = new int[n];
        int* decoutp = new int[n];
        incoutp[0]=1;
        decoutp[n-1]=1;
        for(int i=1;i<n;i++){
            incoutp[i]=1;
            for(int j=i-1;j>=0;j--){
                ...
                int possible = incoutp[j]+1;
                if(possible>incoutp[i]){
                    incoutp[i]=possible;
                }
            }
        }
        ...
        for(int i=n-2;i>=0;i--){
            ...
            for(int j=i+1;j<=n-1;j++){
                ...
                int possible = decoutp[j]+1;
                if(possible>decoutp[i]){
                    decoutp[i]=possible;
                }
            }
        }
