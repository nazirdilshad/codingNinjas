...
int check(int o,int c,int total,bool a[]){
    int currindex = o+c+1,n=total,ans;
    if(dp[o][c]!=-1){
        return dp[o][c];
    }
    if(o>n || c>n){
    ...
    if(o==c || a[currindex]==true){
        ans= check(o+1,c,total,a);
    }
    else if(o==n){
        ans= check(o,c+1,total,a);
    }
    else{
        ans = check(o+1,c,total,a) + check(o,c+1,total,a);
    }
